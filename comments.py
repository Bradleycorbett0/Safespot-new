...
class CommentsScreen(Screen):
        ...
        if os.path.exists(comments_file):
            try:
                ...
                for comment in sorted(comments_data, key=lambda x: x.get('spot_name', '').lower()):
                    ...
                    lbl.bind(
                        on_touch_down=lambda instance, touch, c=comment:
                        self.show_comment_popup(c) if instance.collide_point(*touch.pos) else None
                    )
    ...
    def open_add_comment_popup(self, instance):
        """Open popup for adding a new comment."""
        Window.softinput_mode = 'pan'  # ✅ Moves popup above keyboard

        box = BoxLayout(orientation='vertical', spacing=10, padding=10)

        # Smaller text boxes
        spot_input = TextInput(
            hint_text="Spot Name",
            multiline=False,
            size_hint_y=None,
            height=40
        ...
        # ✅ Bind Enter key + button press
        save_btn.bind(on_release=save_comment_action)
        spot_input.bind(on_text_validate=save_comment_action)
        comment_input.bind(on_text_validate=save_comment_action)

        cancel_btn.bind(on_release=popup.dismiss)
        popup.open()

        # ✅ Auto-focus first text field
        Clock.schedule_once(lambda dt: setattr(spot_input, 'focus', True), 0.3)

    def save_comment(self, spot_name, comment):
        """Save comment to local JSON file"""
        comments_file = "comments.json"
        comments = []
        if os.path.exists(comments_file):
            with open(comments_file, "r") as file:
    ...
    def show_comment_popup(self, comment_data):
        ...
        popup = Popup(title="Comment Options", content=box, size_hint=(0.85, 0.5), background_color=(0, 0, 0, 0.7))
    ...
    def delete_comment(self, target_comment):
        ...
        if os.path.exists(comments_file):
            ...
            updated_comments = [
                c for c in comments
                if not (c["spot_name"] == target_comment["spot_name"] and c["comment"] == target_comment["comment"])
            ]
